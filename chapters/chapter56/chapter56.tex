...
A consistent and modular software toolchain was used to support all stages of development, from data preparation to model training and deployment. Key tools and frameworks included Python (main programming language), Google Colab and Jupyter Notebook (development environments), TensorFlow and PyTorch (deep learning frameworks), Albumentations and OpenCV (data preprocessing and augmentation), scikit-learn (for classical ML tasks), CVAT (for annotation), and Flask, React, and Tailwind CSS (for deployment and UI development). For more details on the tools, libraries, and configurations, see Appendix \ref{app:annexC}.
