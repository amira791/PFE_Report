...
In this section, we introduce the core principles of deep learning, focusing on Deep neural network basics (DNN) and their role in learning complex patterns from data. We explore the structure and components of neural networks, the learning process, optimization techniques, and model evaluation methods. Additionally, we highlight regularization strategies to prevent overfitting.
...
An \gls{abb:af} is a mathematical function applied to a neuron's output in a neural network to introduce non-linearity. Without an activation function, a neural network with multiple layers would behave like a single-layer perceptron, limiting its ability to model complex relationships. Activation functions decide whether a neuron should be activated based on its input. Here are some used activation functions in \gls{abb:dnn} models \parencite{dubey2022activation}:
...
\begin{itemize}
    ...
    \item \textbf{\gls{abb:relu} (Rectified linear unit)}: Widely used in hidden layers due to its simplicity and effectiveness in handling the vanishing gradient problem. It outputs zero for negative inputs and the input itself for positive values, as defined in equation~\ref{eq:relu}.
    \begin{equation}
        \text{\gls{abb:relu}}(x) = \max(0, x)
        \label{eq:relu}
    \end{equation}
...
\section{Convolutional neural networks for classification task}

\gls{abb:cnn} have revolutionized image-based plant disease classification by automatically extracting hierarchical features from agricultural images. Unlike traditional machine learning approaches that rely on handcrafted features, \gls{abb:cnn}s learn spatial patterns directly from raw images, improving classification accuracy. Their ability to recognize disease symptoms from leaf textures and color variations makes them particularly effective in precision agriculture. This section explores the fundamental components of \gls{abb:cnn}s, their advantages in agricultural applications, and their limitations when applied independently.

\subsection{Fundamentals of \gls{abb:cnn}s}
\gls{abb:cnn}s consist of multiple layers designed to process and learn spatial hierarchies from image data (See figure~\ref{fig:figure04}). Their architecture typically includes \parencite{alzubaidi2021review}:


     \begin{figure}[H] % 'H' needs \usepackage{float}
        \centering
        \includegraphics[width=0.9\textwidth]{chapters/chapter1/images/CNNarchitecture.png}
        \caption{Architecture of \gls{abb:cnn} \parencite{Ang2022Optimal}.}
        \label{fig:figure04}
    \end{figure}
...
\subsection{\gls{abb:cnn} architectures for image classification}
Over the past decade, numerous \gls{abb:cnn} architectures have been developed, each introducing unique design principles to improve accuracy, efficiency, and scalability. In the context of image classification, especially for tasks such as plant disease detection, the choice of architecture can significantly influence performance depending on the dataset size, complexity, and computational constraints.
This section presents an overview of some of the most influential and widely used \gls{abb:cnn} architectures:
\subsubsection{Visual geometry group network (VGGNet)}
VGGNet is a \gls{abb:cnn} architecture widely recognized for its simplicity and strong performance in image recognition tasks \parencite{alzubaidi2021review}.
...
Other \gls{abb:cnn} architectures, such as InceptionNet, Xception, and MobileNet, have also been widely adopted for various computer vision tasks. For further details about these architectures, you can refer to the appendix \ref{app:annexA}.
...
%     \label{fig:figure14}
% \end{figure}
The two primary approaches to transfer learning are:
...
Many architectures are designed to efficiently and accurately detect objects in images, even in complex agricultural environments. Below, we discuss four of the most widely used and effective object detection models: YOLO (You Only Look Once), R-\gls{abb:cnn}, Faster R-\gls{abb:cnn}, and SSD (Single Shot Multibox Detector). 
...
At test time, class-specific confidence scores are computed by multiplying the box confidence with conditional class probabilities. YOLO optimizes a loss function during training to fine-tune predictions and improve detection accuracy.
...
\begin{itemize}
    ...
    \item \textbf{Classification and localization:} Region proposals are classified using pre-trained linear Support Vector Machine (SVM)s, and bounding box regression is applied. Non-maximum suppression (NMS) is used to eliminate redundant boxes and finalize object detections.
\end{itemize}
...
It integrates predictions from multiple feature maps with different resolutions and uses a VGG16 backbone architecture with additional layers for bounding box predictions. SSD is trained with a combination of localization and confidence losses and refines detections using non-maximum suppression (NMS). It outperforms Faster R-\gls{abb:cnn} in accuracy on PASCAL VOC and COCO while being three times faster, running at 59 fps with an input size of 300 × 300. However, SSD still struggles with small objects, which can be improved with better feature extractors and network modifications \parencite{zhao2019objectdetection}.
...
Lastly, ensuring the interpretability and trustworthiness of \gls{abb:ai} decisions is crucial in agricultural contexts, as these technologies directly impact yield, resource use, and farmers’ livelihoods. Addressing these challenges requires collaborative efforts in data collection, annotation, model adaptation, and efficient deployment strategies \parencite{alzubaidi2021review}.

\section{Conclusion}

This chapter laid the foundations of deep learning by introducing its key concepts, architectures, and learning strategies, with a particular focus on image classification and object detection tasks. We presented the main \gls{abb:cnn} architectures, explaining how each contributes to improved performance in visual tasks. In addition, we discussed the concept of transfer learning as a practical solution to adapt pre-trained models to agricultural contexts.
